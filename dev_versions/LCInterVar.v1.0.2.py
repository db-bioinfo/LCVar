...
def check_PVS1(line,Funcanno_flgs,Allels_flgs,lof_genes_dict):
    ...
    # Parse SnpEff LOF annotation from Otherinfo field
    snpeff_lof_genes = set()
    nmd_genes = set()
    
    try:
        otherinfo = cls[Funcanno_flgs.get('Otherinfo', -1)]
        if otherinfo != '.' and 'LOF=' in otherinfo:
            # Extract LOF annotations: LOF=(GENE|GENE|1|1.00)
            import re
            lof_matches = re.findall(r'LOF=\(([^|]+)\|[^|]+\|[^|]+\|([^)]+)\)', otherinfo)
            ...
            nmd_matches = re.findall(r'NMD=\(([^|]+)\|[^|]+\|[^|]+\|([^)]+)\)', otherinfo)
